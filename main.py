...
from tasks.task2_numbers import dividing_operations, square, sum_numbers
from tasks.task3_lists import get_identical_elems, get_list_average
from tasks.task4_dictionaries import get_dictionary_keys, get_united_dictionary
from tasks.task5_sets import check_if_set_contains, united_sets
...
def main():
    ...
    print(
        f"Task 2 - integer_division_part: {integer_division_part}, reminder: {reminder}"
    )
    print("Task 3:")
    list1 = [num1, num2]
    list2 = [num1, num2]
    ...
    print(
        f"Task 5 - set 2 is a subset of the set1: {check_if_set_contains(set1, set2)}"
    )
    ...
    lambda_even_or_odd = lambda n: "even" if n % 2 == 0 else "odd"  # noqa: E731
    print(
        f"Task 7 - Lambda func which check even or odd for {num2} say: this is {lambda_even_or_odd(num2)}"
    )
